...
@Command(name = "loop", description = "Loops the current track")
public class Loop extends BaseSlashVoiceChannelCommandHandler {

    @Override
    protected void handle() throws BotException {
        boolean loop = MusicService.loop(voiceChannel);
        interaction.createImmediateResponder()
                .addEmbed(
                        new EmbedBuilder()
                                .setTitle(getMessage(server.getPreferredLocale(), String.format("command.loop.%b.title", loop)))
